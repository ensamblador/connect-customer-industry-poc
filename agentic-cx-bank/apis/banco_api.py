"""
apis/banco_api.py — Amazon API Gateway REST API for the banking backend.

Following the project convention, ALL API Gateway constructs for the project
live under `apis/`. This module exposes the banking self-service REST surface
...
class BancoApi(Construct):
    """REST API for the banking self-service backend."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.api = apigw.RestApi(
            ...
            description="Banking self-service backend (accounts, products, cards).",
    ...
    def _set_up_api_key(self) -> None:
        ...
        self.api_key_secret = sm.Secret(
            self,
            "ApiKeySecret",
            description="Banking self-service API key (API Gateway + AgentCore).",
        ...
        self.api_key = self.api.add_api_key(
            "BancoApiKey",
            api_key_name="banco-selfservice-api-key",
            value=key_value,
        )
        self.usage_plan = self.api.add_usage_plan(
            "BancoUsagePlan", name="banco-selfservice-usage-plan"
        )
