...
_ASSIST_RETRIEVE_INSTRUCTION = "Use this to answer questions about accounts, cards, transfers, fees and commissions, activating a card, branch hours, and FAQs. Phrase the query as a complete, specific question, not keywords."
...
def _retrieve_tool_dict(association_id, tag_filters, instruction=_RETRIEVE_INSTRUCTION, examples=_RETRIEVE_EXAMPLES) -> dict:
    ...
    Bedrock/Q-in-Connect RetrievalFilter grammar) so e.g. industry=bank AND
    language=es restricts retrieval to the Spanish banking articles.
...
def build_tools(
    ...
    *,
    ...
    (industry=bank) and, when `content_language` is set, ALSO by
    ...
    tools = [retrieve]
    tools += [_mcp_tool_dict(t, mcp_tool_prefix, request_card_confirmation) for t in BANK_MCP_TOOLS]
