"""
agentic_cx_bank/contact_flows_stack.py — Phase 5: contact flow modules + flows.

The full inbound + escalation chain, CDK-managed:

    inbound flow ──FlowModuleId──▶ set-customer-session-banco module ─▶ ai_session Lambda
                 ──FlowModuleId──▶ banco-escalate-to-agent module ─DefaultAgentUI▶ screen-pop flow ─▶ handoff view
...
class ContactFlowsStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        self.handoff_view = CustomerManagedView(
            ...
            actions=["Submit"],
            description="Banking escalation hand-off screen-pop view.",
        )
        ...
        # --- set-customer-session-banco module (invokes ai_session Lambda) ---
        ...
        self.inbound = ContactFlow(
            ...
            "InboundFlow",
            instance_arn=instance_arn,
            name="banco-selfservice-es-inbound",
            flow_path=os.path.join(_ROOT, config.FLOW_INBOUND),
            flow_type="CONTACT_FLOW",
            description="Banking Spanish self-service inbound voice flow (CDK-managed).",
            replacements={
