"""
agentic_cx_bank/ai_agents_stack.py — Phase 4: AI prompts + agents.
...
class AiAgentsStack(Stack):
    ...
    def _create_agents(self, assistant_id: str) -> None:
        ...
        self.voice_agent = OrchestrationAIAgent(
            self,
            "VoiceAgent",
            agent_name="banco-selfservice-voice-es",
            prompt_version_id=self.voice_prompt.ai_prompt_version_id,
            surface=AgentSurface.VOICE,
            description="Banco voice-first self-service orchestration agent (es-US).",
            **common,
        )
        ...
        self.chat_agent = OrchestrationAIAgent(
            self,
            "ChatAgent",
            agent_name="banco-selfservice-chat-es",
            prompt_version_id=self.chat_prompt.ai_prompt_version_id,
            surface=AgentSurface.CHAT,
            description="Banco chat-first self-service orchestration agent (es-US).",
            **common,
        )
