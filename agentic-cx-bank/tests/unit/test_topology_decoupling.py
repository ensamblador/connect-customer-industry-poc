...
Structural guards for the Bank_Project's six-phase, SSM-decoupled CDK topology.
These tests synthesize the app exactly as ``app.py`` wires it and assert:

  * exactly six stacks with the exact ``CX-BANCO-*`` names, each defined in the
    ``agentic_cx_bank`` package (Requirements 1.1, 1.2, 1.7);
...
EXPECTED_EDGES = {
    "CX-BANCO-MCP": set(),
    "CX-BANCO-KB": set(),
    "CX-BANCO-CONNECT-SUPPORT": {"CX-BANCO-MCP", "CX-BANCO-KB"},
    "CX-BANCO-AGENTS": {"CX-BANCO-KB", "CX-BANCO-MCP"},
    "CX-BANCO-FLOWS": {"CX-BANCO-MCP", "CX-BANCO-CONNECT-SUPPORT", "CX-BANCO-AGENTS"},
    "CX-BANCO-WEBSITE": {"CX-BANCO-FLOWS", "CX-BANCO-MCP"},
}
...
def _build_app() -> tuple[cdk.App, dict[str, cdk.Stack]]:
    """Instantiate the six CX-BANCO-* stacks and the five edges, mirroring app.py."""
    app = cdk.App()
...
# --------------------------------------------------------------------------- #
# Requirements 1.1, 1.2, 1.7 — exactly six CX-BANCO-* stacks in the package
# --------------------------------------------------------------------------- #
...
def test_all_stacks_defined_in_agentic_cx_bank_package(built):
    _, stacks, _ = built
    for name, stack in stacks.items():
        module = type(stack).__module__
        assert module.startswith("agentic_cx_bank"), (
            f"{name} ({type(stack).__name__}) is defined in {module!r}, "
            "expected the agentic_cx_bank package"
        )
