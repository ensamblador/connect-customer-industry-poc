"""
tests/unit/test_kb_template.py — CX-BANCO-KB (Phase 2) template + coverage asserts.

Two complementary layers of guard for the banking knowledge base:
  ...
  1. CDK template assertions — synthesize a fresh ``KnowledgeBaseStack`` and
     ...
     template:
       ...
       * the KB ``Tags`` carry ``industry = bank`` (Requirements 5.1, 10.2).
  ...
  2. Coverage / manifest asset checks — read the repo's KB assets directly from
     the filesystem (no synth) and assert:
       ...
       * the manifest declares ``resourceTags == {"industry": "bank"}``
         (Requirement 10.2);
...
# Project root (agentic-cx-bank/): tests/unit/<this file> -> parents[2].
PROJECT_ROOT = Path(__file__).resolve().parents[2]
KB_ROOT = PROJECT_ROOT / "knowledge_bases" / "bank"
...
def template() -> Template:
    ...
    app = cdk.App()
    stack = KnowledgeBaseStack(app, "CX-BANCO-KB")
...
# --------------------------------------------------------------------------- #
# Requirements 5.1, 10.2 — the KB carries the industry=bank resource tag
# --------------------------------------------------------------------------- #

def test_knowledge_base_tags_include_industry_bank(template: Template):
    kbs = template.find_resources("AWS::Wisdom::KnowledgeBase")
    (kb,) = kbs.values()
    tags = kb["Properties"].get("Tags", []) or []
    tag_pairs = {(t["Key"], t["Value"]) for t in tags}
    assert ("industry", "bank") in tag_pairs, (
        f"KB Tags must include industry=bank; found {sorted(tag_pairs)}"
    )
...
# --------------------------------------------------------------------------- #
# Requirement 10.2 — manifest declares the base tag industry=bank
# --------------------------------------------------------------------------- #

def test_manifest_declares_industry_bank_resource_tag(manifest: dict):
    assert manifest.get("resourceTags") == {"industry": "bank"}, (
        f"manifest resourceTags must equal {{'industry': 'bank'}}; "
