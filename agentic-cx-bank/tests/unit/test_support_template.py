"""
tests/unit/test_support_template.py — CX-BANCO-CONNECT-SUPPORT (Phase 3) asserts.

Two complementary layers of guard for the banking Connect support resources:
  ...
  1. CDK template assertions — synthesize a fresh ``ConnectSupportStack``
     ...
     CloudFormation template:
       ...
       * each security profile carries the MCP application grant — a single
         ...
         the nine ``<target>___<op>`` banking tool ids, namespaced on the
       ...
       * the four published SSM parameters exist under ``/agentic-cx-bank``:
         ``SP_SELFSERVICE_ID``, ``SP_ASSIST_ID``, ``VIEW_NEWLINE_ARN``, and
         ``LEX_BOT_ALIAS_ARN`` (Requirement 6.6).

  2. View structure-preservation diff — read each banking view and its telco
     ...
     differing (Requirements 6.9, 6.10):
       * ``banco-card-request-form``      vs telco ``telco-newline-form``;
       * ``banco-card-activation-guide``  vs telco ``telco-esim-activation-guide``.
...
# The nine banking MCP tool ids the profile grant must reference: the gateway
...
# (bank view, telco source view) pairs for the structure-preservation diff.
VIEW_PAIRS = {
    "card-request-form": (
        BANK_ROOT / "views" / "banco-card-request-form" / "view-content.json",
        TELCO_ROOT / "views" / "telco-newline-form" / "view-content.json",
    ),
    "card-activation-guide": (
        BANK_ROOT / "views" / "banco-card-activation-guide" / "view-content.json",
...
def template() -> Template:
    ...
    app = cdk.App()
    stack = ConnectSupportStack(app, "CX-BANCO-CONNECT-SUPPORT")
...
def test_each_security_profile_has_mcp_grant(template: Template):
    ...
    for logical_id, profile in profiles.items():
        ...
        # The nine banking tool ids, exact and in order.
        assert app.get("ApplicationPermissions") == EXPECTED_MCP_TOOL_IDS, (
            f"{logical_id} MCP tool ids != the nine banking <target>___<op> ids"
        )
...
# --------------------------------------------------------------------------- #
# Requirement 6.6 — the four published SSM values under /agentic-cx-bank
# --------------------------------------------------------------------------- #
...
def test_four_published_ssm_params_exist(template: Template):
    ...
    # Every published name is under the banking namespace.
    for name in EXPECTED_SSM_NAMES:
        assert name.startswith("/agentic-cx-bank"), (
            f"published SSM name {name!r} is outside the /agentic-cx-bank namespace"
        )
...
def test_view_source_files_exist(pair: str):
    bank_path, telco_path = VIEW_PAIRS[pair]
    assert bank_path.is_file(), f"missing banking view {bank_path}"
...
def test_view_id_set_is_identical(pair: str):
    ...
    assert _view_ids(bank) == _view_ids(telco), (
        f"{pair}: _id set diverged — "
        f"only-in-bank={sorted(_view_ids(bank) - _view_ids(telco))}, "
        f"only-in-telco={sorted(_view_ids(telco) - _view_ids(bank))}"
    )
...
def test_view_component_tree_is_identical(pair: str):
    ...
    bank = json.loads(bank_path.read_text(encoding="utf-8"))
    telco = json.loads(telco_path.read_text(encoding="utf-8"))
    assert _body_trees(bank) == _body_trees(telco), (
...
def test_view_content_actually_differs(pair: str):
    ...
    telco = json.loads(telco_path.read_text(encoding="utf-8"))
    assert bank != telco, (
        f"{pair}: banking view is byte-identical to the telco source — "
        "content was not re-themed"
    )
    # And no telco token should survive in the banking view's content.
    assert "telco" not in json.dumps(bank).lower(), (
        f"{pair}: a 'telco' substring survives in the banking view content"
    )
