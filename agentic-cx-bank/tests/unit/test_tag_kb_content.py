...
call is ever made:
  ...
  1. ``_detect_language`` — infers the per-item ``language`` tag (es | pt | en)
     from a content item's ``bank/<lang>/`` source path segment, and returns
...
@pytest.mark.parametrize(
    "name, expected",
    [
        ("bank/es/activar-tarjeta.txt", "es"),
        ("bank/pt/activar-tarjeta.txt", "pt"),
        ("bank/en/activar-tarjeta.txt", "en"),
    ],
)
def test_detect_language_from_bank_lang_segment(name, expected):
    """A ``bank/<lang>/`` path segment yields the matching language code."""
...
def test_detect_language_falls_back_to_title_and_metadata():
    ...
    by_title = {"name": "no-language-here.txt", "title": "bank/en/faq.txt"}
    assert tag_kb_content._detect_language(by_title, LANGUAGES) == "en"

    by_metadata = {
        "name": "no-language-here.txt",
        "metadata": {"sourcePath": "bank/es/cuentas.txt"},
    }
...
@pytest.mark.parametrize(
    "name",
    [
        "bank/activar-tarjeta.txt",          # no language segment at all
