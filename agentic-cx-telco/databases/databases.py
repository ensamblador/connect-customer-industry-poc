"""
databases/databases.py — DynamoDB tables for the telco self-service backend.
...
Three tables back the API:
  ...
  * plans     (PK planId)                             — mobile plan catalog
  * lines     (PK lineId,     GSI customerId-index)   — new-line requests;
                                                        lookup by customer
...
class Tables(Construct):
    """All DynamoDB tables for the telco-cx project (+ sample data)."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # plans — PK planId; small catalog, scanned/filtered for listing.
        ...
        # lines — PK lineId; GSI to list a customer's new-line requests.
        self.lines = dynamodb.Table(
            self,
            "LinesTable",
            table_name=lines_table_name,
            partition_key=dynamodb.Attribute(
                name="lineId", type=dynamodb.AttributeType.STRING
            ),
            **_TABLE_CONFIG,
        )
        self.lines.add_global_secondary_index(
        ...
        self._load_sample_data("lines", self.lines)
    ...
    def get_all_tables(self) -> list[dynamodb.Table]:
        return [self.accounts, self.plans, self.lines]
