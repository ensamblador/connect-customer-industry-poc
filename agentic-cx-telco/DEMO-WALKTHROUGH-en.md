# Demo Script — Telco PoC

> 🌐 **Languages:** [Español](./DEMO-WALKTHROUGH.md) · **English** (this file)

Step-by-step guide for demonstrating the telco PoC capabilities. It gives you the questions to use and what to expect at each step, without scripting the agent's replies (they are generated live and vary).
...
| Customer | Email | Phone | Plan | Balance | Status |
|----------|-------|-------|------|---------|--------|
| María González | maria.gonzalez@example.com | +12065550101 | Unlimited 5G ($70/mo) | $42.50 | active |
| James Carter | james.carter@example.com | +12065550102 | Plus ($45/mo) | $0 | active |
| Aisha Khan | aisha.khan@example.com | +12065550103 | Basic ($25/mo) | $88.00 | suspended |
| Diego Fernández | diego.fernandez@example.com | +12065550104 | Plus ($45/mo) | $15.75 | active |

**Available plans:**
- Telco Basic: 5 GB, $25/mo (no hotspot)
- Telco Plus: 25 GB, $45/mo (with hotspot)
- Telco Unlimited 5G: 100 GB, $70/mo (with hotspot, priority access)

> The data can be viewed on the website at the `/datos` route ("Datos demo" link in the navigation).
...
1. Open the CloudFormation output of the **CX-TELCO-WEBSITE** stack → take the value of `WebsiteDistributionDomainName` (the CloudFront domain, e.g. `https://d1234abcdef.cloudfront.net`). The `WebsiteDataViewerPath` output gives you the `/datos` URL directly.
2. Browse to the site. You will see the "Latam Telco" page with plans, coverage and help sections.
...
The agent answers these questions **from the knowledge base articles**, not from the model's own knowledge. The articles live in `knowledge_bases/telco/entries/<language>/`, one folder per language (`es`, `en`, `pt`): the agent retrieves the entry matching the language the customer is speaking.

Questions to try:

> **You:** How can I use my phone abroad?

> **You:** How do I turn on WiFi calling?

> **You:** Do you have 5G coverage in my area?

> **You:** What time do the stores open?

> **You:** How do I know if my phone is compatible with you?
...
**More questions to keep exploring** (same sources, still inside the KB): eSIM activation, number portability, what each plan includes, hotspot usage, data overage charges.

---

### Demo 2: Account lookup (MCP Tools)

These questions trigger MCP tools that query the database in real time.

> **You:** How much do I owe on my bill?

> **You:** Which plan am I on?

> **You:** What plans do you offer?

**What to expect:** the agent first resolves the account from the session email, then invokes the MCP tool. The answer may be something like the **outstanding balance and its due date**, pulled live from the API via MCP. Cross-check against `/datos` that the values match the customer's record.

**Follow-up questions to try:**

> **You:** When is my next payment due?

> **You:** Does my plan include hotspot?

> **You:** How much would it cost me to switch to Unlimited 5G?

> **You:** Do I have any additional lines on my account?

---

### Demo 3: Request a new line (guided form)
...
> **You:** I want to add a new line to my account
...
**What to expect:** the agent picks the conversation back up, confirms the request, and gives you the new line's identifier with its initial status.
...
> **You:** Can I see my requested lines?
...
> **You (speaking):** "Hi, I want to know how to turn on WiFi calling on my phone"

**What to expect:** a spoken answer based on the KB articles, in the same language you asked in.

**Other questions to try by voice:** roaming abroad, 5G coverage, store hours, device compatibility.

#### Account lookup by voice

> **You:** "I want to know how much I owe"

**What to expect:** it answers with the balance and its due date, in a natural voice.

**Other questions to try:** which plan you are on, what plans are available, when the next payment is due.

### 2.3 Line signup by voice (separate test)
...
> **You:** "I want a new line"

**What to expect:** the agent presents the plan options, and before executing the action it asks for an **explicit confirmation** (user confirmation is enabled on voice). Only once you confirm does it create the request and return the new line's identifier.

**Verification:** browse to `/datos` on the website → you will see the new line with status **`requested`**.

### 2.4 Test with a phone number (optional)

For a demo with a real phone call and automatic customer recognition:

1. In the **DynamoDB** console → `telco-accounts` table → edit one of the test records (e.g. Diego Fernández) and replace `phoneNumber` with **your real phone number** in E.164 format (e.g. `+573001234567`). This lets the flow identify you automatically when you call, without asking who you are.
2. In the **Amazon Connect** console → **Phone numbers** → claim a phone number (DID).
3. Associate it with the self-service contact flow (the deployed inbound flow, `telco-selfservice-es-inbound`).
...
> **You:** Hi, I have a problem with my bill. I'm being charged $15.75 but I already paid last month. I need you to adjust the balance.

**What to expect:** the agent recognizes that billing adjustments are out of its scope, explains it, and announces the transfer to a representative.
...
1. **Immediate screen-pop** with the "escalated contact" view showing:
   ...
   - **Customer intent:** balance adjustment for a charge they believe is already paid
   - **Escalation summary:** (AI-generated) — what the customer asked, what self-service tried, why a human is needed
   - **Recommended action:** verify the payment and adjust the balance if applicable
...
> the AI agent (`telco-agent-assist-iac`) to hold the permissions: the human user
...
> | **Views** | `CustomViews.Access` | the eSIM step-by-step guide (4.2) |
> | **MCP tools** | a `Type: MCP` application on the profile, with namespace = gateway id and the nine `telco-rest-api-oas-target___<operation>` ids | live data lookups (4.4) |
>
> The simplest path is to assign the human user the same **`telco-agent-assist-iac`** profile that Phase 3 deploys (its id is published to SSM as `SP_ASSIST_ID`), or to add those permissions and the MCP grant to their current profile.
>
...
> **Customer:** "Also, I want to know if I can activate an eSIM on my new phone"

**In the Agent Assist panel you will see:**
- An answer with the eSIM activation information and a link to the KB entry (from the `esim-activacion.txt` article)
- A **guide button "Activar eSIM"** suggested automatically

### 4.2 The step-by-step guide (eSIM)

Click the **"Activar eSIM"** button in the suggestions panel.
...
> **Agent types:** "What are the Miami store hours?"

**What to expect:** the answer with that store's details taken from the KB store-hours article.
...
> **Agent types:** "What lines do they have active?"

**What to expect:** live account data (balance and due date, lines and their status), without the agent leaving the workspace or opening another tool.
...
> **You:** I want to cancel my line

**What to expect:** the agent explains that cancellations are handled by a representative and escalates with reason `out_of_scope`.

### 5.3 Suspended account

Sign in as `aisha.khan@example.com` and ask:

> **You:** Why can't I make calls?
...
### 5.4 Specific plan lookup

> **You:** How much data does the Basic plan include?

### 5.5 Number portability

> **You:** Can I keep my number if I switch to you?

**What to expect:** an answer based on the KB portability article.
...
> **You:** Gracias. ¿Qué plan tengo?

Try Portuguese:

> **You:** Quais planos vocês oferecem?
...
> **You (switching to Spanish):** "Sí, ¿me puedes decir qué plan tengo?"
...
- [ ] The new-line form appears when requesting a line
- [ ] The requested line shows up in `/datos` with status `requested`
- [ ] Web call works and the voice sounds natural
- [ ] Line signup by voice asks for explicit confirmation before executing
...
- [ ] The eSIM step-by-step guide renders correctly
