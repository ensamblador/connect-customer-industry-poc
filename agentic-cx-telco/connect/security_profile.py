...
The telco self-service agent's only data tool is the KB **Retrieve** tool, so
the least-privilege profile carries a single permission: **`Wisdom.View`**.
This mirrors the existing `telco-selfservice-ai-agent` profile on the instance
(verified: its sole permission is `Wisdom.View`).
