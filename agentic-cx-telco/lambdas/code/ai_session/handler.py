"""
ai-session-telco Lambda handler.

Invoked synchronously by the ``set-customer-session-telco`` contact flow module
(InvokeLambdaFunction, STRING_MAP). Given the captured contact endpoint
(``email`` or ``phoneNumber``) it:

  1. Looks the customer up in the telco-accounts DynamoDB table — by phone via
...
them back. This is how the new-line plan picker persists ``selectedPlan``,
but any future attribute can be written the same way with no code change —
the flow just passes it as a parameter.
...
Environment:
  ACCOUNTS_TABLE        - telco-accounts table name
...
def handler(event, _context):
    ...
    # (e.g. selectedPlan from the new-line plan picker). Any future attribute
    # can be written this way with no Lambda code change — the flow just
