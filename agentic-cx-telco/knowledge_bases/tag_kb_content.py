...
`Retrieve` tool filters by a content tag (e.g. `industry=telco`), so after every
...
3. Derives a per-item `language` tag (es | pt | en) from each item's
   `telco/<lang>/` source path and applies it ALONGSIDE the base segmentation
...
-----
    ...
    python tag_kb_content.py --tags industry=telco,visibility=public
...
_DEFAULT_MANIFEST = os.path.join(_HERE, "telco", "manifest.json")
...
def _parse_tags(raw: str | None, manifest: dict) -> dict[str, str]:
    ...
    return manifest.get("resourceTags") or {"industry": "telco", "visibility": "public"}


def _detect_language(item: dict, languages: list[str]) -> str | None:
    """Infer a content item's language from its source path segments.

    The EXTERNAL S3 crawler keys content under telco/<lang>/..., so the item's
...
def main() -> int:
    ...
    kb_id = args.kb_id  or _resolve_kb_id_from_ssm()
    print (f"KB:{kb_id}")
    if not kb_id:
        sys.exit(
