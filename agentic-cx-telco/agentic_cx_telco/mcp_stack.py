"""
agentic_cx_telco/mcp_stack.py — Phase 1: the MCP / AgentCore foundation.

This is the first phased stack of the agentic-cx-telco sample. It composes the
telco self-service backend and exposes it to Amazon Connect AI agents as an MCP
server, then publishes the ids/ARNs/URLs the downstream phases need onto the SSM
"bus" (shared/ssm_names.py). Nothing here imports another phase or emits a
...
Published SSM parameters (only values that cross a stack boundary):
    ...
    plans + ai_session Lambda ARNs (-> Phase 5 flows).
...
class McpStack(Stack):
    ...
    def create_resources(self) -> None:
        ...
        self.tables = Tables(
            ...
            plans_table_name=config.PLANS_TABLE_NAME,
            lines_table_name=config.LINES_TABLE_NAME,
        )
        ...
        self.api = TelcoApi(
            ...
            plans_fn=self.lambdas.plans,
            lines_fn=self.lambdas.lines,
        ...
        self.gateway = AgentCoreGateway(
            ...
            description="Telco self-service MCP server (accounts, plans, lines).",
        )
        ...
        self.openapi_target = self.gateway.add_openapi_inline_target(
            ...
            description="Telco REST API (rich inline OpenAPI) as MCP tools.",
        )
        ...
        if config.HAS_REAL_INSTANCE:
            self.mcp_integration = McpServerIntegration(
                ...
                # readable prefix + unique suffix, e.g. telco-mcp-server-<id>)
                ...
                description="Telco self-service MCP server for Connect AI agent tools.",
            )

        # 5b. Associate the plans + ai_session Lambdas with the Connect instance
        # (IntegrationType: LAMBDA_FUNCTION) so contact-flow "Invoke Lambda"
        # blocks (Phase 5) can call them; the association also adds the
        ...
        if config.HAS_REAL_INSTANCE:
            ...
            self.plans_lambda_integration = LambdaConnectIntegration(
                self,
                "PlansLambdaIntegration",
                instance_arn=instance_arn,
                function=self.lambdas.plans,
            )
    ...
    def set_up_env_vars(self) -> None:
        ...
        # plans: catalog.
        self.lambdas.plans.add_environment("PLANS_TABLE", self.tables.plans.table_name)
        # lines: request/list/get, list-by-customer via GSI.
        self.lambdas.lines.add_environment("LINES_TABLE", self.tables.lines.table_name)
        self.lambdas.lines.add_environment(
            "LINES_CUSTOMER_INDEX", self.tables.CUSTOMER_INDEX_NAME
        )
    ...
    def set_up_permissions(self) -> None:
        # Least-privilege: accounts + plans read-only; lines reads and writes.
        self.tables.accounts.grant_read_data(self.lambdas.accounts)
        self.tables.plans.grant_read_data(self.lambdas.plans)
        self.tables.lines.grant_read_write_data(self.lambdas.lines)
    ...
    def publish(self) -> None:
        ...
        Only the values that genuinely cross a stack boundary are published:
          ...
          * the plans + ai_session Lambda ARNs — invoked by the Phase 5 flows.
        Everything else (gateway url, REST API id/url, accounts/lines ARNs,
        ...
        ssm_names.publish(self, "PLPlan", ssm_names.LAMBDA_PLANS_ARN, self.lambdas.plans.function_arn)
        ssm_names.publish(self, "PLSess", ssm_names.LAMBDA_AI_SESSION_ARN, self.lambdas.ai_session.function_arn)
