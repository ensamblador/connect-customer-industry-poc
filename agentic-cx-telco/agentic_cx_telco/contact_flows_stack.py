"""
agentic_cx_telco/contact_flows_stack.py — Phase 5: contact flow modules + flows.

The full inbound + escalation chain, CDK-managed:

    inbound flow ──FlowModuleId──▶ set-customer-session-telco module ─▶ ai_session Lambda
                 ──FlowModuleId──▶ escalate-to-agent module ─DefaultAgentUI▶ screen-pop flow ─▶ handoff view
...
class ContactFlowsStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        self.handoff_view = CustomerManagedView(
            ...
            actions=["Submit"],
            description="Telco escalation hand-off screen-pop view.",
        )
        ...
        # --- set-customer-session-telco module (invokes ai_session Lambda) ---
        self.set_session_module = ContactFlowModule(
            self,
            "SetCustomerSessionModule",
            instance_arn=instance_arn,
            name="set-customer-session-telco",
        ...
        self.inbound = ContactFlow(
            ...
            flow_type="CONTACT_FLOW",
            description="Telco Spanish self-service inbound voice flow (CDK-managed).",
            replacements={
