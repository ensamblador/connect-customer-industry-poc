"""
agentic_cx_telco/connect_support_stack.py — Phase 3: Connect supporting resources.
...
(Phase 5) depend on, but which are siblings of each other:
  ...
  * Customer-managed views — the new-line guided form and the eSIM activation
    guide.
...
class ConnectSupportStack(Stack):
    """Phase 3 — security profiles + customer-managed views + eSIM guide flow."""
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        self.view_newline = CustomerManagedView(
            ...
            description="Telco new-line guided request form for chat self-service.",
        )
        self.view_esim = CustomerManagedView(
            self,
            "EsimGuideView",
            instance_arn=instance_arn,
        ...
        # --- eSIM guide contact flow (references the eSIM view) ---
        self.esim_flow = ContactFlow(
            self,
            "EsimGuideFlow",
            instance_arn=instance_arn,
            ...
            replacements={
                "ESIM_GUIDE_VIEW_ARN_PLACEHOLDER": self.view_esim.view_qualified_arn,
            },
        )
        self.esim_flow.node.add_dependency(self.view_esim)
        ...
        CfnOutput(self, "EsimGuideViewArn", value=self.view_esim.view_qualified_arn)
        CfnOutput(self, "EsimGuideFlowArn", value=self.esim_flow.flow_arn)
