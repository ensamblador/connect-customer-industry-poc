"""
agentic_cx_telco/knowledge_base_stack.py — Phase 2: the Q in Connect knowledge base.

Builds one EXTERNAL Wisdom knowledge base from the repo's plain-text (.txt)
entries under
knowledge_bases/telco/entries/<lang>/ (es, pt, en — uploaded together under the
telco/ prefix), associates it with the Q in Connect AI Agents domain, and
publishes the two values later phases consume:

    KB_ID        — Phase 3 binds the eSIM guide content association to it
...
Two operational notes:
  ...
  * AWS::Wisdom::KnowledgeBase has NO update path. To rebuild the KB (e.g. after
    changing the content source), tear this stack down and redeploy it:
        cdk destroy CX-TELCO-KB
        cdk deploy  CX-TELCO-KB
  ...
  * Content TAGGING is a post-deploy step, NOT a CDK resource. The S3/EXTERNAL
    ...
    Retrieve tool filters by the `industry=telco` tag AND a per-item `language`
...
class KnowledgeBaseStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # (all languages
        # under knowledge_bases/telco/entries/<lang>/). entries_dir /
        ...
        # per-content-item segmentation tags (industry=telco + per-language
