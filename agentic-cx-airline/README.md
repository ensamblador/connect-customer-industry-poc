...
>
> Este README documenta **solo lo específico de la aerolínea**. Lo común a las tres industrias vive en los documentos maestros de arriba y no se repite aquí.
...
- **Vistas administradas por el cliente** (`AWS::Connect::View`): el formulario guiado de
  reserva de vuelo (`AirlineReservationForm`) y la guía de maleta perdida
...
| | |
|---|---|
| Stacks, en orden de fase | `CX-AIRLINE-MCP` · `CX-AIRLINE-KB` · `CX-AIRLINE-CONNECT-SUPPORT` · `CX-AIRLINE-AGENTS` · `CX-AIRLINE-FLOWS` · `CX-AIRLINE-WEBSITE` |
| Perfiles de seguridad a adjuntar | voz + chat → `airline-selfservice-ai-agent` · agent-assist → `airline-agent-assist-iac` |
| Flujo de la guía a asociar | `Reportar maleta perdida` → contenido `maleta-perdida` de la KB |
| Bot Lex a compilar | `airline-qconnect-bot-v2` (`en_US`, `es_US`, `pt_BR`) |

> **Particularidad de esta app:** la habilitación de los locales en el TestBotAlias la maneja automáticamente un custom resource en `CX-AIRLINE-CONNECT-SUPPORT`, así que el chat funciona sin la alternancia manual de locales del alias. Aun así hay que **compilar** los tres locales en la consola de Lex.
