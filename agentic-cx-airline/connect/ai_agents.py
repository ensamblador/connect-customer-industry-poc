...
Escalate/Complete RTC, plus ShowCardRequestGuide (chat only).
"""
...
# the cited KB content (e.g. activar-tarjeta). See the guide spec design
...
_ASSIST_RETRIEVE_INSTRUCTION = "Use this to answer questions about reservations, the frequent-flyer program and miles, check-in, baggage, airports, lost or delayed baggage, and general FAQs. Phrase the query as a complete, specific question, not keywords."
...
_ESCALATE_INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "escalationReason": {"type": "string", "description": "Category of the escalation reason.", "enum": ["billing_question", "transaction_or_service_issue", "account_inquiry_failed", "out_of_scope", "customer_request", "customer_frustration", "technical_issue", "other"]},
    },
...
def _retrieve_tool_dict(association_id, tag_filters, instruction=_RETRIEVE_INSTRUCTION, examples=_RETRIEVE_EXAMPLES) -> dict:
    ...
    Bedrock/Q-in-Connect RetrievalFilter grammar) so e.g. industry=airline AND
    language=es restricts retrieval to the Spanish airline articles.
...
def _mcp_tool_dict(spec: dict, prefix: str, request_card_confirmation: bool) -> dict:
    if spec["name"] == "requestCard":
        confirmation = request_card_confirmation
    else:
...
def _show_card_request_guide_tool_dict() -> dict:
    return {
        "toolName": "ShowCardRequestGuide",
        "toolType": "RETURN_TO_CONTROL",
        "description": (
            "Ends the AI turn and returns control to the flow to display a "
            "guided card request form in the chat window. Chat only."
        ),
        "instruction": _instr(_SHOW_CARD_REQUEST_GUIDE_INSTRUCTION, [
        ...
        "inputSchema": _SHOW_CARD_REQUEST_GUIDE_INPUT_SCHEMA,
        "userInteractionConfiguration": _uic(False),
    }
...
def build_tools(
    ...
    *,
    ...
    """Compose the clean tool-dict list for an agent surface.
      ...
      Chat  : Retrieve + 9 MCP + Escalate + Complete + ShowCardRequestGuide (requestCard confirm OFF)
      Assist: Retrieve + 9 MCP only                                      (no handoff tools)

    The Retrieve tool filters KB content by `content_tag_key=content_tag_value`
    (industry=airline) and, when `content_language` is set, ALSO by
    ...
    self-service one.
    """
    # requestCard user-confirmation: ON for voice (verbal gate) and assist; OFF
    # for chat (the ShowCardRequestGuide form is the explicit confirmation).
    request_card_confirmation = surface is not AgentSurface.CHAT
    ...
    tools = [retrieve]
    tools += [_mcp_tool_dict(t, mcp_tool_prefix, request_card_confirmation) for t in AIRLINE_MCP_TOOLS]
    ...
    if surface is AgentSurface.CHAT:
        tools.append(_show_card_request_guide_tool_dict())
    return tools
