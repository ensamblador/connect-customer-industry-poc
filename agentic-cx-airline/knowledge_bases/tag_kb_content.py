...
`Retrieve` tool filters by a content tag (e.g. `industry=airline`), so after every
...
3. Derives a per-item `language` tag (es | pt | en) from each item's
   `airline/<lang>/` source path and applies it ALONGSIDE the base segmentation
...
-----
    ...
    python tag_kb_content.py --tags industry=airline,visibility=public
...
_DEFAULT_MANIFEST = os.path.join(_HERE, "airline", "manifest.json")
...
def _parse_tags(raw: str | None, manifest: dict) -> dict[str, str]:
    ...
    return manifest.get("resourceTags") or {"industry": "airline", "visibility": "public"}


def _detect_language(item: dict, languages: list[str]) -> str | None:
    """Infer a content item's language from its source path segments.

    The EXTERNAL S3 crawler keys content under airline/<lang>/..., so the item's
