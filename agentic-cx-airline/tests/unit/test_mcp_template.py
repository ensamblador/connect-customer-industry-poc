"""
tests/unit/test_mcp_template.py — CX-AIRLINE-MCP (Phase 1) template assertions.
...
contract against the resulting CloudFormation template:
  ...
  * every published SSM parameter name starts with ``/agentic-cx-airline``
...
# airline backend functions from the incidental provider functions.
...
def template() -> Template:
    ...
    app = cdk.App()
    stack = McpStack(app, "CX-AIRLINE-MCP")
...
def test_published_ssm_params_use_airline_namespace(template: Template):
    ...
    for logical_id, param in params.items():
        ...
        assert isinstance(name, str) and name.startswith("/agentic-cx-airline"), (
            f"{logical_id} publishes SSM name {name!r} outside the "
            "/agentic-cx-airline namespace"
        )
...
def test_no_telco_substring_in_resource_names(template: Template):
    ...
    assert "telco" not in rendered.lower(), (
        "found a 'telco' substring in the synthesized CX-AIRLINE-MCP template"
    )
