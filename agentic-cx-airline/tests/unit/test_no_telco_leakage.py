...
Final validation that the airline re-theme left no telecommunications token
anywhere a customer-facing airline artifact, configuration value, or
...
The guard has three independent parts:
  ...
  1. Shipping-asset file scan (Requirements 2.1, 2.3, 12.6) — walk the SHIPPING
     source tree of ``agentic-cx-airline/`` (Python, JSON, YAML, txt, md, js, html,
     css, config) and assert no case-insensitive ``telco`` substring appears in
     any file. Airline data, Lambda handlers, OpenAPI, KB documents, flow/view
     JSON, prompt YAML, the website source, and the README are all in scope.

  2. Config airline-value scan (Requirements 2.1, 2.3) — import ``config`` and
     assert every configured airline value (resource names, table names, view /
  ...
  3. Synthesized-template scan (Requirements 2.1, 2.3) — synthesize each of the
     six in-scope ``CX-AIRLINE-*`` stacks and assert no ``telco`` substring
...
Scope / exclusions:
  * ``tests/`` is excluded — the template/structure-preservation suites
    ...
    references, not airline-asset leakage.
  ...
  * The only allowed cross-project dependency is the external ``/flows/init/es``
    ...
    zero telco tokens remain anywhere in the airline project source.
...
# tests/unit/<this file> -> parents[2] == agentic-cx-airline/.
...
def test_shipping_scope_actually_scans_files():
    ...
    covered = {p.relative_to(AIRLINE_ROOT).as_posix() for p in files}
    ...
    assert any(p.startswith("knowledge_bases/airline/") for p in covered)


def test_no_telco_token_in_shipping_assets():
    """No airline asset (data, code, OpenAPI, KB docs, flow/view JSON, prompt
    ...
    for path in _iter_shipping_files():
        ...
        if TELCO in text.lower():
            rel = path.relative_to(AIRLINE_ROOT).as_posix()
            offenders.append(f"{rel} (lines {_telco_line_numbers(text)})")

    assert not offenders, (
        "telco token leaked into airline shipping assets:\n  "
        + "\n  ".join(sorted(offenders))
    )


# --------------------------------------------------------------------------- #
# Part 2 — config airline-value scan (Requirements 2.1, 2.3)
# --------------------------------------------------------------------------- #
...
def test_config_airline_values_have_no_telco():
    """Every configured airline value — resource names, table names, view/flow/
    ...
    assert not offenders, (
        "telco token found in configured airline values:\n  "
        + "\n  ".join(sorted(offenders))
    )
...
STACK_FACTORIES = {
    "CX-AIRLINE-MCP": McpStack,
    "CX-AIRLINE-KB": KnowledgeBaseStack,
    "CX-AIRLINE-CONNECT-SUPPORT": ConnectSupportStack,
    "CX-AIRLINE-AGENTS": AiAgentsStack,
    "CX-AIRLINE-FLOWS": ContactFlowsStack,
    "CX-AIRLINE-WEBSITE": WebsiteStack,
}
