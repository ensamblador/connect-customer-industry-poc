"""
tests/unit/test_agents_template.py — CX-AIRLINE-AGENTS (Phase 4) asserts.

Two complementary layers of guard for the airline AI agents:
  ...
  1. CDK template assertions — synthesize a fresh ``AiAgentsStack``
     ...
     assert the Phase-4 contract against the resulting CloudFormation template:
       * exactly three orchestration prompts (``AWS::Wisdom::AIPrompt``) and
         ...
         ``airline-*`` identifier (Requirements 7.1, 7.2, 7.3);
  ...
  2. Prompt structure-preservation check — read each airline prompt YAML and its
...
# The three airline prompt identifiers (Requirement 7.3) and agent identifiers.
EXPECTED_PROMPT_NAMES = {
    "airline-selfservice-voice-orchestration",
    "airline-selfservice-chat-orchestration",
    "airline-agent-assist-orchestration",
}
EXPECTED_AGENT_NAMES = {
    "airline-selfservice-voice-es",
    "airline-selfservice-chat-es",
    "airline-agent-assist-es",
}

# The nine airline MCP tool names bound to every surface (Requirement 7.4/7.6).
MCP_TOOL_NAMES = {
    "getAccountByPhone",
    "getAccountByEmail",
    "getAccount",
...
EXPECTED_TOOLS_BY_AGENT = {
    "airline-selfservice-voice-es": VOICE_TOOLS,
    "airline-selfservice-chat-es": CHAT_TOOLS,
    "airline-agent-assist-es": ASSIST_TOOLS,
}
...
# (airline prompt, telco source prompt) pairs for the structure-preservation check.
PROMPT_PAIRS = {
    "voice": (
        AIRLINE_ROOT / config.AI_AGENT_VOICE_PROMPT,
        TELCO_ROOT
        / "connect_ai_agents/telco-selfservice-voice/prompts/telco-selfservice-orchestration-voice.yaml",
    ),
    "chat": (
        AIRLINE_ROOT / config.AI_AGENT_CHAT_PROMPT,
        TELCO_ROOT
        / "connect_ai_agents/telco-selfservice-chat/prompts/telco-selfservice-orchestration-chat.yaml",
    ),
    "assist": (
        AIRLINE_ROOT / config.AI_AGENT_ASSIST_PROMPT,
        TELCO_ROOT
...
def template() -> Template:
    ...
    app = cdk.App()
    stack = AiAgentsStack(app, "CX-AIRLINE-AGENTS")
...
# --------------------------------------------------------------------------- #
# Requirement 7.3 — airline identifiers on prompts and agents
# --------------------------------------------------------------------------- #

def test_prompt_identifiers_are_airline(template: Template):
...
def test_agent_identifiers_are_airline(template: Template):
...
def test_agent_tool_counts(template: Template):
    ...
    assert counts == {
        "airline-selfservice-voice-es": 12,
        "airline-selfservice-chat-es": 13,
        "airline-agent-assist-es": 10,
...
def test_prompt_source_files_exist(pair: str):
    airline_path, telco_path = PROMPT_PAIRS[pair]
    assert airline_path.is_file(), f"missing airline prompt {airline_path}"
...
def test_prompt_top_level_keys_identical(pair: str):
    airline_path, telco_path = PROMPT_PAIRS[pair]
    airline = _load_yaml(airline_path)
    telco = _load_yaml(telco_path)
    assert set(airline.keys()) == set(telco.keys()), (
        f"{pair}: top-level YAML keys diverged — "
        f"airline={sorted(airline.keys())}, telco={sorted(telco.keys())}"
    )
...
def test_prompt_messages_shape_identical(pair: str):
    ...
    airline = _load_yaml(airline_path)
    telco = _load_yaml(telco_path)
    assert _messages_shape(airline) == _messages_shape(telco), (
        f"{pair}: messages shape diverged — "
        f"airline={_messages_shape(airline)}, telco={_messages_shape(telco)}"
    )
...
def test_prompt_section_markers_identical(pair: str):
    ...
    airline_path, telco_path = PROMPT_PAIRS[pair]
    airline_markers = _section_markers(_load_yaml(airline_path))
    telco_markers = _section_markers(_load_yaml(telco_path))

    # Same number of markers.
    assert len(airline_markers) == len(telco_markers), (
        f"{pair}: section-marker count diverged — "
        f"airline={len(airline_markers)}, telco={len(telco_markers)}"
    )
    # Same structural skeleton (order + open/close nesting), tolerating a
    # re-themed section name that keeps the same position.
    assert _marker_structure(airline_markers) == _marker_structure(telco_markers), (
        f"{pair}: section structure diverged from the telco source — "
        f"airline={airline_markers}, telco={telco_markers}"
    )
    # Any marker that is not byte-identical must be a re-themed *name only* —
    # never a telco token surviving in the airline prompt.
    for marker in airline_markers:
...
def test_prompt_content_actually_rethemed(pair: str):
    ...
    an accidental verbatim copy that would leave telco wording in a airline prompt."""
    airline_path, telco_path = PROMPT_PAIRS[pair]
    airline = _load_yaml(airline_path)
    telco = _load_yaml(telco_path)
    assert airline != telco, (
        f"{pair}: airline prompt is identical to the telco source — not re-themed"
    )
    assert "telco" not in json.dumps(airline).lower(), (
        f"{pair}: a 'telco' substring survives in the airline prompt content"
    )
