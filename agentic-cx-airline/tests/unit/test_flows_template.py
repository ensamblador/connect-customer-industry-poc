"""
tests/unit/test_flows_template.py — CX-AIRLINE-FLOWS (Phase 5) asserts.

Three complementary layers of guard for the airline contact flows / modules:
  ...
  1. CDK template assertions — synthesize a fresh ``ContactFlowsStack``
     ...
     assert the Phase-5 contract against the resulting CloudFormation template:
       * exactly two ``AWS::Connect::ContactFlow`` — ``airline-agent-screenpop-es``
         and ``airline-selfservice-es-inbound`` (Requirement 8.1);
       * exactly two ``AWS::Connect::ContactFlowModule`` —
         ``airline-escalate-to-agent`` and ``set-customer-session-airline``
         (Requirement 8.1);
       * every provisioned flow/module name is airline-themed and matches no
         live telco flow name (Requirements 8.1, 8.7).

  2. Flow structure-preservation diff — read each airline flow JSON and its
     ...
     id becomes ``set-customer-session-airline`` — is normalized away in the
...
# The four airline flow/module names the stack provisions (Requirement 8.1).
EXPECTED_CONTACT_FLOW_NAMES = {
    "airline-agent-screenpop-es",
    "airline-selfservice-es-inbound",
}
EXPECTED_FLOW_MODULE_NAMES = {
    "airline-escalate-to-agent",
    "set-customer-session-airline",
}
EXPECTED_ALL_NAMES = EXPECTED_CONTACT_FLOW_NAMES | EXPECTED_FLOW_MODULE_NAMES

# The live telco flow/module names on the shared instance. No airline flow name
...
# InvokeFlowModule action was renamed telco -> airline. Normalizing the airline id
# back to the telco id lets the structural diff treat everything else as a
# content-only retheme (Requirement 8.11).
RENAME_AIRLINE_TO_TELCO = {"set-customer-session-airline": "set-customer-session-telco"}

# (airline flow JSON, telco source flow JSON) pairs for the structure diff.
FLOW_PAIRS = {
    "screenpop": (
        AIRLINE_ROOT / "flows" / "airline-agent-screenpop-es" / "flow.json",
        TELCO_ROOT / "flows" / "telco-agent-screenpop-es" / "flow.json",
    ),
    "escalate-module": (
        AIRLINE_ROOT / "flows" / "airline-escalate-to-agent" / "flow.json",
        TELCO_ROOT / "flows" / "escalate-to-agent" / "flow.json",
    ),
    "set-customer-session": (
        AIRLINE_ROOT / "flows" / "set-customer-session-airline" / "flow.json",
        TELCO_ROOT / "flows" / "set-customer-session-telco" / "flow.json",
    ),
    "inbound": (
        AIRLINE_ROOT / "flows" / "airline-selfservice-es-inbound" / "flow.json",
        TELCO_ROOT / "flows" / "telco-selfservice-es-inbound" / "flow.json",
    ),
}

# All four airline flow JSONs, for the flow-language structural validation.
AIRLINE_FLOWS = {name: pair[0] for name, pair in FLOW_PAIRS.items()}
...
HANDOFF_VIEW = AIRLINE_ROOT / config.ESCALATION_HANDOFF_VIEW_CONTENT
...
def template() -> Template:
    ...
    app = cdk.App()
    stack = ContactFlowsStack(app, "CX-AIRLINE-FLOWS")
...
def _norm(identifier: str) -> str:
    """Canonicalize an action id by undoing the one intended airline->telco rename."""
    return RENAME_AIRLINE_TO_TELCO.get(identifier, identifier)
...
def _transition_edges(flow: dict) -> set[tuple[str, str, str]]:
    ...
    airline/telco graphs line up node-for-node.
    """
...
# --------------------------------------------------------------------------- #
# Requirement 8.7 — airline names collide with no live telco flow name
# --------------------------------------------------------------------------- #
...
def test_no_flow_name_matches_telco(template: Template):
    ...
    assert not collisions, f"airline flow names collide with telco: {sorted(collisions)}"
...
def test_flow_source_files_exist(pair: str):
    airline_path, telco_path = FLOW_PAIRS[pair]
    assert airline_path.is_file(), f"missing airline flow {airline_path}"
...
def test_flow_identifier_set_is_identical(pair: str):
    airline_path, telco_path = FLOW_PAIRS[pair]
    airline = _identifier_set(_load_flow(airline_path))
    telco = _identifier_set(_load_flow(telco_path))
    assert airline == telco, (
        f"{pair}: Identifier set diverged (after rename-normalization) — "
        f"only-in-airline={sorted(airline - telco)}, only-in-telco={sorted(telco - airline)}"
    )
...
def test_flow_transition_graph_is_identical(pair: str):
    airline_path, telco_path = FLOW_PAIRS[pair]
    airline = _transition_edges(_load_flow(airline_path))
...
def test_no_telco_token_survives_in_flow(pair: str):
    """No airline flow — generic or re-themed — may leak a 'telco' token."""
    airline_path, _ = FLOW_PAIRS[pair]
    airline = _load_flow(airline_path)
    assert "telco" not in json.dumps(airline).lower(), (
        f"{pair}: a 'telco' substring survives in the airline flow content"
    )
...
def test_content_bearing_flow_actually_rethemed(pair: str):
    ...
    (or the telco module name) in a airline flow."""
    airline_path, telco_path = FLOW_PAIRS[pair]
    airline = _load_flow(airline_path)
    telco = _load_flow(telco_path)
    assert airline != telco, (
        f"{pair}: airline flow is byte-identical to the telco source — not re-themed"
    )
...
@pytest.mark.parametrize("name", sorted(AIRLINE_FLOWS), ids=sorted(AIRLINE_FLOWS))
def test_flow_language_is_structurally_valid(name: str):
    _validate_flow_language(_load_flow(AIRLINE_FLOWS[name]))
...
def test_inbound_flow_accepts_friendly_name_identifiers():
    ...
    accept them as-is."""
    flow = _load_flow(AIRLINE_FLOWS["inbound"])
