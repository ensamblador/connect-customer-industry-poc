"""
agentic_cx_airline/mcp_stack.py — Phase 1: the MCP / AgentCore foundation.

This is the first phased stack of the agentic-cx-airline sample. It composes the
airline self-service backend and exposes it to Amazon Connect AI agents as an
...
per-resource-type constructs verbatim:
    ...
    AirlineApi               apis/airline_api.py            (L2 apigateway.RestApi + key)
...
from apis.airline_api import SECRET_API_KEY_JSON_KEY, AirlineApi
...
class McpStack(Stack):
    ...
    def create_resources(self) -> None:
        ...
        self.openapi_target = self.gateway.add_openapi_inline_target(
            ...
            description="Airline REST API (rich inline OpenAPI) as MCP tools.",
        )
        ...
        if config.HAS_REAL_INSTANCE:
            self.mcp_integration = McpServerIntegration(
                ...
                # readable prefix + unique suffix, e.g. airline-mcp-server-<id>)
                ...
                description="Airline self-service MCP server for Connect AI agent tools.",
            )
