"""
agentic_cx_airline/knowledge_base_stack.py — Phase 2: the Q in Connect knowledge base.

Builds one EXTERNAL Wisdom knowledge base from the repo's plain-text (.txt)
entries under
knowledge_bases/airline/entries/<lang>/ (es, pt, en — uploaded together under the
airline/ prefix), associates it with the Q in Connect AI Agents domain, and
...
Two operational notes:
  ...
  * AWS::Wisdom::KnowledgeBase has NO update path. To rebuild the KB (e.g. after
    changing the content source), tear this stack down and redeploy it:
        cdk destroy CX-AIRLINE-KB
        cdk deploy  CX-AIRLINE-KB
  ...
  * Content TAGGING is a post-deploy step, NOT a CDK resource. The S3/EXTERNAL
    ...
    Retrieve tool filters by the `industry=airline` tag AND a per-item `language`
...
class KnowledgeBaseStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # (all languages
        # under knowledge_bases/airline/entries/<lang>/). entries_dir /
        ...
        # per-content-item segmentation tags (industry=airline + per-language
