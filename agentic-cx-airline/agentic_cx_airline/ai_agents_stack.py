"""
agentic_cx_airline/ai_agents_stack.py — Phase 4: AI prompts + agents.
...
that bind each published prompt version. Each prompt keeps its own model
(voice/chat = Haiku 4.5 global, assist = Sonnet global).
...
Tool surfaces:
    voice  : Retrieve + 9 MCP + Escalate + Complete           (createReservation confirm ON)
    chat   : voice surface + ShowReservationGuide             (createReservation confirm OFF)
...
class AiAgentsStack(Stack):
    ...
    def _create_agents(self, assistant_id: str) -> None:
        ...
        self.voice_agent = OrchestrationAIAgent(
            self,
            "VoiceAgent",
            agent_name="airline-selfservice-voice-es",
            prompt_version_id=self.voice_prompt.ai_prompt_version_id,
            surface=AgentSurface.VOICE,
            description="Airline voice-first self-service orchestration agent (es-US).",
            **common,
        )
        ...
        self.chat_agent = OrchestrationAIAgent(
            self,
            "ChatAgent",
            agent_name="airline-selfservice-chat-es",
            prompt_version_id=self.chat_prompt.ai_prompt_version_id,
            surface=AgentSurface.CHAT,
            description="Airline chat-first self-service orchestration agent (es-US).",
            **common,
        )
        ...
        self.assist_agent = OrchestrationAIAgent(
            self,
            "AssistAgent",
            agent_name="airline-agent-assist-es",
            prompt_version_id=self.assist_prompt.ai_prompt_version_id,
            surface=AgentSurface.ASSIST,
            description="Airline agent-assistance orchestration agent (es_US).",
            **assist_common,
        )
