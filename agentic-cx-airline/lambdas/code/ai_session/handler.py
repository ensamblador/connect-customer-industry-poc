"""
ai-session-airline Lambda handler.

Invoked synchronously by the ``set-customer-session-airline`` contact flow module
(InvokeLambdaFunction, STRING_MAP). Given the captured contact endpoint
(``email`` or ``phoneNumber``) it:

  1. Looks the customer up in the airline-accounts DynamoDB table — by phone via
...
Environment:
  ACCOUNTS_TABLE        - airline-accounts table name
