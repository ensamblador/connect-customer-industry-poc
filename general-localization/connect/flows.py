...
the authored JSON in `projects/general-localization/flows/<name>/flow.json` and
load it verbatim, so the repo file stays the single source of truth and the
designer export round-trips.

Optional templating: a flow's JSON often embeds values that should be resolved
at synth — ARNs (the instance, a view, a queue, the AI-agents domain) or, for
this project, the per-language hold-message text and TTS voices. Pass
`replacements={"<MARKER>": <value>}` and author the JSON with the markers;
`str.replace` swaps them at synth. When the referenced resources are
pre-existing, the JSON can carry their literal ARNs and no replacement is
needed.
