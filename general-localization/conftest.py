...
``config.py`` reads ``INSTANCE_ID`` / ``ASSISTANT_ID`` from the environment via
``config_validation.require`` and raises ``ConfigError`` when either is missing,
so the suite would fail at import time in a shell that has not sourced the
repo-root ``.env``. The tests only ever SYNTHESIZE CloudFormation templates —
they never call AWS — so placeholder ids are sufficient and keep the suite
hermetic. (``INSTANCE_ALIAS`` is unused by this app; it is set anyway so one
conftest body is shared verbatim across all four apps.)
